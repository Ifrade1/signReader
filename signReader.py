...
#termios, tty,
import numpy as np #extra library for python for scientific computation and linear algebra
import numpy.linalg 
    ...
    if(statusOf['P'] == 'down'): #PINKY IS DOWN 
        ...
        if (statusOf['M'] == 'down'): #IL:Pointing down?
            ...
            if (statusOf['I'] == 'up'):
                ...
                return
            if (statusOf['R'] is not 'up'): #IL:thumb on pinky, all other fingers curled on top.
        ...
        if (statusOf['R'] == 'up'): #RING IS EXTENDED
            if (statusOf['I'] == 'up'): #INDEX IS EXTENDED
                     if ((statusOf['T'] is not 'out') and (angleBetween['IM'] <= (touchingMargin+10))): #THUMB IS DOWN AND FINGERS ARE CLOSE
                        print "You signed a B" #IL: pinky, ring, middle, index finger all pointed up. thumb down against palm. 
                        return
                     else:
                        print "That's an open palm. High five!"
                        return
            else: #INDEX IS NOT EXTENDED
                print "You signed an F" #IL:pinky, ring, & middle up and touching. thumb and index touching.
                return
...
def main():
    ...
    currentFPS = 0 #IL: current frame rate
